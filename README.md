# Lunaris Mutex Library

This is a mutex library that you can add on your project! It works on both Windows and Linux.
...
FetchContent_Declare(
    lunaris-mutex
    GIT_REPOSITORY https://github.com/LunarisLib/lunaris-mutex.git
    GIT_TAG        (put version here)
)
FetchContent_MakeAvailable(lunaris-mutex)
```
...
find_package(lunaris-mutex REQUIRED)
...
The find_package will try to find the `lunaris-mutex-config.cmake` or similar files that should be available to download in the Release tab.